...
class Router
{
  ...
  public function prepareRoute($parts)
  {
    ...
    foreach ($parts as $part) {
      $keyword = strtolower((string)$part);

      if ($keyword === '')
        continue;

      $resolvedQuery = $this->getQueryByKeyword($keyword);
      ...
      [$queryKey, $queryValue] = array_pad(explode('=', $resolvedQuery, 2), 2, null);
      ...
      if ($queryKey === 'category_id') {
        if (!isset($this->request->get['path'])) {
          $this->request->get['path'] = $queryValue;
        } else {
      ...
      if ($queryKey === 'blog_category_id') {
        if (!isset($this->request->get['blog_category_id'])) {
          $this->request->get['blog_category_id'] = $queryValue;
        } else {
  ...
  private function resolveFinalRoute()
  {
    ...
    if (isset($this->request->get['article_id'])) {
      ...
      $blogCategoryPath = $this->getBlogPathByArticle($this->request->get['article_id']);
      if ($blogCategoryPath) {
        $this->request->get['blog_category_id'] = $blogCategoryPath;
      }
  ...
  /**
   * Генерация SEO-пути для ссылки: принимает data (route + product_id, path, article_id и т.д.) и language_id.
   * По data['route'] подготавливает data: для product/product — подставляет path главной категории товара;
   * для product/category — нормализует path в полную цепочку категорий; для blog/article — путь блог-категории;
   * для review/agree и т.п. — сразу возвращает [url, data, postfix] без переписывания.
   * Собирает из data список queries (product_id=, path=, blog_category_id=, …), для каждого получает keyword
   * через getKeywordByQuery, склеивает в URL. Для пустых queries (например common/home) ищет keyword по route.
   * Возвращает [url, data, postfix].
   */
  ...
  /**
   * Рекурсивно строит цепочку category_id от корня до category_id: по parent_id поднимается вверх,
   * накапливает путь в current_path, возвращает массив id (корень … категория).
   */
  ...
  /**
   * Заполняет cat_tree (для каждой category_id — путь родительских id), keywords и queries
   * из БД seo_url (или из кэша, если config_seo_url_cache). Нужно для getQueryByKeyword/getKeywordByQuery и getPathByCategory.
   */
  private function initHelpers()
  {
    ...
    if (!$this->cat_tree || empty($this->cat_tree)) {
      ...
      $all_cat_query = $this->db->query(
        'SELECT category_id, parent_id FROM ' . DB_PREFIX . 'category ORDER BY parent_id',
      );
    ...
    if ($this->config->get('config_seo_url_cache')) {
      ...
      if (!$this->keywords || empty($this->keywords) || !$this->queries || empty($this->queries)) {
        ...
        $sql =
          'SELECT ' . $sql_keyword . ', query, store_id, language_id FROM ' . DB_PREFIX . 'seo_url WHERE 1';
  ...
  /**
   * Удаляет из request->get['_route_'] суффикс страницы (config_page_postfix), если он задан и совпадает с концом строки.
   */
  private function detectPostfix()
  {
    if ($this->config->get('config_page_postfix') && isset($this->request->get['_route_'])) {
      $this->request->get['_route_'] = preg_replace(
        '/' . $this->config->get('config_page_postfix') . '$/',
        '',
        (string) $this->request->get['_route_'],
      );
    }
  }

  /**
   * Добавляет к url суффикс страницы (config_page_postfix), если он задан в настройках.
   */
  ...
  /**
   * По ключевому слову (slug) из URL возвращает query из seo_url (например category_id=5, product_id=12)
   * для текущего store_id и language_id. Сначала смотрит кэш queries, иначе запрос в БД.
   */
  ...
  /**
   * По query (например category_id=5 или product_id=12) возвращает keyword (slug) из seo_url
   * для текущего store_id и language_id. Сначала кэш keywords, иначе БД. Учитывает config_seopro_lowercase.
   */
  ...
  /**
   * Проверка канонического URL: строит канонический SEO-адрес для текущих route и get-параметров через url->link.
   * Сравнивает с текущим REQUEST_URI. Если не совпадают — редирект 301 на канонический. Не выполняется для CLI, POST, AJAX, фидов (sitemap и т.д.).
   */
  public function validate()
  {
    ...
    if ($secure) {
      $host = substr(
        ...
        0,
        $this->strpos_offset('/', $this->config->get('config_ssl'), 3) + 1,
      );
    } else {
      $host = substr(
        $this->config->get('config_url'),
        0,
        $this->strpos_offset('/', $this->config->get('config_url'), 3) + 1,
      );
    }
    ...
    $seo = str_replace(
      '&amp;',
      '&',
      $this->url->link($route, $this->getQueryString(['_route_', 'route']), $secure),
    );
  ...
  //         DB_PREFIX .
  //         "language WHERE code = '" .
  //         (int) $this->session->data['language'] .
  //         "' AND status = '1' LIMIT 1",
  //     );
  //     if ($query->num_rows) {
  ...
  /**
   * Возвращает полный путь категорий для товара (id_id_id от корня до главной категории товара).
   * Главная категория берётся из product_to_category по main_category DESC. Использует getPathByCategory и кэш product_categories.
   */
  ...
  private function getBlogPathByArticle($article_id)
  {
    ...
    $query = $this->db->query(
      'SELECT blog_category_id FROM ' .
        DB_PREFIX .
        "article_to_blog_category WHERE article_id = '" .
        (int) $article_id .
        "' ORDER BY main_blog_category DESC LIMIT 1",
    );
    $blog_category_path = $this->getBlogPathByCategory(
      $query->num_rows ? (int) $query->row['blog_category_id'] : 0,
    );
  ...
  /**
   * Возвращает путь блог-категории в виде строки id_id (от корня до blog_category_id) через рекурсивные JOIN в БД.
   * Результат кэшируется в static и при включённом config_seo_url_cache — в cache.
   */
    ...
    if (!isset($blog_path[$blog_category_id])) {
      ...
      for ($i = 1; $i < $max_level; ++$i) {
        $sql .=
          ' LEFT JOIN ' .
          DB_PREFIX .
          "blog_category t$i ON (t$i.blog_category_id = t" .
          ($i - 1) .
          '.parent_id)';
      }
  ...
  /**
   * Возвращает позицию occurrence-го вхождения needle в haystack (0 = первое). Для построения базового хоста из config_url/config_ssl.
   */
